class AttendancesController < ApplicationController
  ...
  def update_attendance_management_info
    ...
    if params[:attendance][:work_start_time].to_i < 10
      ...
      redirect_back(fallback_location: attendance_management)
    elsif params[:attendance][:work_start_time].present? && params[:attendance][:break_start_time].present? && !(params[:attendance][:work_start_time].to_time.hour == params[:attendance][:break_start_time].to_time.hour) && (params[:attendance][:break_start_time].to_time.min - params[:attendance][:work_start_time].to_time.min).abs >= 45
      flash[:danger] = "出勤時間と休憩開始時間の差分で15分以内の場合は登録できません。"
      redirect_back(fallback_location: attendance_management)
